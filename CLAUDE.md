...
- **`lib/acp/agents.ts`** — the `ACP_AGENTS` registry, the multi-FRAMEWORK seam. Adding an entry is the ONLY change needed to surface another agent (own picker entry, own client/child process, own session history). Each entry's `backend` field says how to spawn it: `{ kind: "hermes", profileName, envHomeKey }` for the Hermes adapter, or `{ kind: "command", command, args }` for ANY ACP-speaking CLI (current entries: Hermes default, OpenCode via `opencode acp`, Antigravity via the `agy-acp` community adapter). Nothing else may hardcode an agent id.
- **`lib/acp/client.ts`** — NDJSON JSON-RPC client (`initialize`, `session/new|list|load|prompt|cancel`; reverse request `session/request_permission`). Multi-session: `activePrompts` keyed by sessionId, per-session latest-wins cancel. The process-global `pendingPermissions` registry stores agent-supplied options; `resolveAcpPermissionByDecision(requestId, approved)` maps a boolean to a concrete optionId (55s auto-deny timeout). `session/list` and `session/load` are optional ACP capabilities: a -32601 from the agent degrades to an empty history / a 404 chat instead of an error.
- **`lib/acp/backend.ts`** — turns each registry entry's `backend` into a spawn spec (Hermes: `HERMES_ACP_HOME` venv python, profiles inject `HERMES_HOME`/`HERMES_PROFILE`; command backends spawn from the user's home dir with inherited env); lazy client singletons per agent id on `globalThis` (HMR-safe).
