...
- **`lib/acp/agents.ts`** — the `ACP_AGENTS` registry, the multi-FRAMEWORK seam. Adding an entry is the ONLY change needed to surface another agent (own picker entry, own client/child process, own session history). Each entry's `backend` field says how to spawn it: `{ kind: "hermes", profileName, envHomeKey }` for the Hermes adapter, or `{ kind: "command", command, args }` for ANY ACP-speaking CLI (current entries: Hermes default, OpenCode via `opencode acp`, Gemini CLI via `gemini --acp`). Nothing else may hardcode an agent id.
